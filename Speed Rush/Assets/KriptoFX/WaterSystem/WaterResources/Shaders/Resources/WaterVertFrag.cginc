...
half4 ComputeWaterColor(v2f i, float facing)
{
	...
	//return float4(normal.xz*10, 0, 1);
...
//#endif
	...
	//half R = saturate((reflVec.y + 0.05) * 10);
	//normal = normalize(lerp(half3(0, 1, 0), normal, R));
	//normalReflection = normalize(lerp(half3(0, 1, 0), normalReflection, R));
	...
	reflection = tex2D(KW_PlanarReflection, refl_uv);
		...
		float2 bounceMaskUV = i.screenPos.xy / i.screenPos.w + normal.xz * 0.5 - float2(0, KW_PlanarReflectionClipPlaneOffset);
		float reflDepth = tex2D(KW_PlanarReflectionDepth, bounceMaskUV);
		reflection = reflDepth > 0.0001 ? tex2D(KW_PlanarReflection, bounceMaskUV).xyz : reflection;
	#endif
#else
	...
	half surfaceTensionFade = saturate((LinearEyeDepth(SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, UNITY_PROJ_COORD(i.screenPos))) - (i.screenPos.w)) * 8);
	...
	underwaterColor = lerp(underwaterColor, clamp(KW_AmbientColor.rgb * 0.125 + volumeScattering.a * 0.5 +  volumeScattering.rgb * 0.25, 0, 0.75), foamTex * foamMask);
